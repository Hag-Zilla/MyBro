...
### Model Loading
- Load models once at startup using FastAPI lifespan or a module-level singleton, never per-request.
- Validate that the loaded model artifact matches the expected version or schema before serving.
...
- Return structured prediction responses with at least `prediction`, `confidence`, and `model_version`.
...
- Use `async def` handlers to avoid blocking the event loop during I/O (model load, disk reads).
...
- Add a test for the invalid-input path (malformed payload should return HTTP 422, not 500).
